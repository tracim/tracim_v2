...
if [ ! -f /etc/tracim/config.ini ]; then
    cp /opt/tracim/backend/development.ini.sample /etc/config.ini
    # Must prepare some lines to be able to use tracim on 127.0.0.1 (and 127.0.0.1 only !)
    sed -i "s|listen = .*|listen = 0.0.0.0:6543|g" /etc/config.ini
    sed -i "s|website.base_url = .*|website.base_url = http://localhost:6543|g" development.ini
